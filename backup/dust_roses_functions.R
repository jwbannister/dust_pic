...
pull_mfile_data <- function(start_date, end_date, teom_sites){
    ...
    mfile_sites <- mfile_names[teom_sites]
...
epa_api_pm10_hourly <- function(start_date, end_date){
    ...
    for (i in 1:length(prd)){
        print(year(prd[[i]][1]))
        url <- paste0("https://aqs.epa.gov/data/api/sampleData/byCounty",
                      "?email=jbannister@airsci.com", 
                      "&key=carmelgazelle52", 
                      "&param=81102", 
                      "&bdate=", format(prd[[i]][1], "%Y%m%d"), 
                      "&edate=", format(prd[[i]][2], "%Y%m%d"), 
                      "&state=06&county=027")
        dfx <- read.csv(url)
        if (i == 1){
            df1 <- dfx
    ...
    df2 <- filter(df1, grepl('TEOM', method.description))
    epa_xwalk <- c('North Beach'=29, 'Lizard Tail'=28, 'Keeler'=1003, 
                   'Mill Site'=30, 'Shell Cut'=25, 'Dirty Sox'=22, 
                   'Stanley'=26, 'Olancha'=21)
    df3 <- filter(df2, site.num %in% epa_xwalk) %>%
        select(site.num, date.local, x24.hour.local, pm10=sample.measurement, 
               sample.duration) %>%
        arrange(site.num, date.local, x24.hour.local)
    df3$site <- sapply(df3$site.num, function(x) names(epa_xwalk)[which(epa_xwalk==x)])
    df3$datetime <- as.POSIXct(paste(df3$date.local, df3$x24.hour.local))
    df3$date <- date(df3$datetime - 1)
    df3$year <- year(df3$date)
    df3 <- df3 %>% select(-date.local, -x24.hour.local)
    return(df3)
}
